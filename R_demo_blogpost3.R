...
mydata$valuea <-
  c(rep(3, 10), c(rep(4, 10))) #unreal data - group A is all 3, and group B is all 4
badd <-
  c(-2, -1, -1, 0, 0, 0, 0, 1, 1, 2) #still unreal, but with some variation
cadd <-
  c(-3, -2, -1, -1, 0, 0, 1, 1, 2, 3) #still unreal, but with more variation
mydata$group <- as.factor(mydata$group)
levels(mydata$group) <- c('A', 'B')
#set up to save plots
if (saveplot == 1) {
  #set saveplot to zero if you want to see in plot window
  pngname <- 'Fig3_1_nonoise_midnoise_bignoise.png'
  png(pngname,
      width = 3300,
      height = 1200,
      res = 300)#needs a lot of fiddly formatting to get decent resolution
}
par(mfrow = c(1, 3)) #print in 1 row and 3 columns
par(mar = c(5, 6, 5, 2)) #fiddle with margins
beeswarm(
  mydata$valuea ~ mydata$group ,
  xlab = '',
  ylab = 'Vocabulary gain',
  col = 'red',
  pch = 20,
  cex.axis = 2.2,
  cex.lab = 2.5,
...
#------------------------------------------------------------------
saveplot <-
  1 #set to zero if you want to see plots in plot window, and set to 1 to save as png file
if (saveplot == 1)
{
...
for (i in 1:9) {
  alldat$mydata <-
    sample(myz, 20) #select 20 values from population at random
  alldat$mydata[11:20] <- alldat$mydata[11:20] + 1
  
  beeswarm(
    alldat$mydata ~ alldat$mygroup,
    ylim = c(-3, 3.5),
    xlab = '',
    ylab = 'Z-score',
    main = i,
...
effsize = .6
myseq <-
  c(1, 1, 0, 1, 0, 0, 0, 1, 1) #predetermine which runs have true effect
#set up to save plots
if (saveplot == 1) {
